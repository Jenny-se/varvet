...
function newItem(): LineItem {
  return { key: ++_key, id: null, productName: '', inventoryId: null, quantity: '1', unitPrice: '', vatRate: '25' }
}
  ...
  async function search(q: string) {
    ...
    const [{ data: invData }, { data: rpData }] = await Promise.all([
      supabase.from('inventory').select('id, product_name, colorway, retail_price')
        ...
        .not('retail_price', 'is', null).order('product_name').limit(8),
      supabase.from('receipt_products').select('*').ilike('name', `%${q}%`)
        .eq('active', true).order('sort_order').limit(5),
    ])
    const invS: Suggestion[] = (invData ?? []).map(item => ({ type: 'inventory', item: item as Pick<InventoryItem, 'id' | 'product_name' | 'colorway' | 'retail_price'> }))
...
export default function EditReceiptPage() {
  ...
  function vatSummary() {
    ...
    return Object.entries(map).filter(([, v]) => v > 0).sort(([a], [b]) => Number(b) - Number(a))
  ...
  async function handleSave() {
    ...
    const { error: receiptErr } = await supabase.from('receipts').update({
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/receipts/${id}`} className="p-1.5 rounded-lg text-warm-400 hover:bg-cream-200 transition-colors">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <h1 className="text-xl font-semibold text-warm-900">Ändra kvitto</h1>
      </div>
            ...
            {items.map(it => (
              <div key={it.key} className="grid grid-cols-[1fr_60px_90px_90px_32px] gap-2 items-start">
                <div className="space-y-1">
                  <ProductSearchCell value={it.productName}
                ...
                <input type="number" min="1" step="1" value={it.quantity} onChange={e => setItem(it.key, { quantity: e.target.value })}
                ...
                <input type="number" min="0" step="0.01" value={it.unitPrice} onChange={e => setItem(it.key, { unitPrice: e.target.value })}
                  placeholder="0" className="px-2 py-1.5 text-sm text-right border border-linen-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-sage-300 w-full" />
